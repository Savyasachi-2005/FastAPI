...
from services.password_reset import send_reset_email,send_confirm_email
...
def reset_password(data: passwordResetConfirm,bg:BackgroundTasks,db:Session = Depends(get_db)):
    ...
    db.add(user)
    db.commit()
    bg.add_task(send_confirm_email, email_to=user.email)
